...
def _area_por_fazenda(g: pd.DataFrame) -> pd.Series:
...
def _build_resumo_mensal(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for mes_ano, g in df.groupby("MES_ANO", dropna=False, sort=True):
        ...
        if "PRECIP_CHIRPS_MM" in g.columns:
            ...
            row["Precipitação Média Ponderada (mm)"] = _weighted_mean(g["PRECIP_CHIRPS_MM"], g["AREA_PRODU"])
...
def _build_resumo_anual(resumo_mes: pd.DataFrame) -> pd.DataFrame:
    ...
    for ano, g in resumo_mes.groupby("ANO", dropna=False, sort=True):
        ...
        if all(c in g.columns for c in ["Precipitação Média Ponderada (mm)", "AREA_PRODU"]):
            row["Precipitação Média Ponderada (mm)"] = _weighted_mean(
...
def _render_painel_metricas(resumo_mes: pd.DataFrame, resumo_ano: pd.DataFrame, filtro_desc: str, periodo_desc: str) -> None:
    ...
    if not resumo_mes.empty and all(c in resumo_mes.columns for c in ["Precipitação Média Ponderada (mm)", "AREA_PRODU"]):
        precip_media_mensal = _weighted_mean(
